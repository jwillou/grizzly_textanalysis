...
#set variables
lowerthreshold = c(10,20) #words that do not appear in at least this many documents will be removed
#datasummary = NULL

####find topics for ideas in Condensed_gov_actors columns (separately)####
for(c in 14:26){ #these are columns of interest
  for(l in 1:length(lowerthreshold)){
    documents = data.frame(docs = data[,c])
    metadata  = data.frame(level = data$Condensed_function) #Condensed_gov_actors Condensed_level Condensed_function
    ...
    text   = textProcessor(documents = documents$docs, metadata = metadata, customstopwords=c("get","like","one","re","said","say","will"))
    ...
    #datasummary = rbind(datasummary,c(colnames(data)[c],lowerthreshold[l],table(t[,2]),sum(table(t[,2])),length(text[[2]])))
